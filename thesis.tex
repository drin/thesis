...
% Dr. Kitts
% MST was coined by the folks (water quality oriented environmental
% microbiologists) who do what should really be called FST, or fecal source
% tracking. I think they wanted to sanitize the term. However, MST could
% easily (in my opinion) be applied to tracking the sources of microbes in any
% situation, thus it could also be a branch of epidemiology, which is tracking
% the sources of pathogenic organisms as the cause of disease. The bacillus
% work thus doesn't quite meet the criteria of MST as originally conceived,
% but it isn't much of a stretch. I think you'll be fine using it in your
% thesis, though you may want to add a sentence about the bacillus work being
% an extension of MST into the realm of food microbiology.

% Dr. Black
% It SHOULD be used for tracking any bacteria, but it is not. 
% I think the key is the use of "source", where animals are considered sources
% of the bacteria that are primarily shed in our feces. I disagree with the
% implications behind this limitation, however. I do not see us as anything
% more than moving environments for them to reproduce (i.e., they do not
% originate from our bowels). Nevertheless, we must conform to the common use
% of the terms, so MST = FIB.
...
\chapter{Introduction}\label{chap:intro}
   ...
   %That is, the choice of the Biology department to use a library-dependent
   %MST technique was made before I joined and worked with the research group,
   %and for brevity, I will be focusing only on the brief introduction of
   %library-dependent methods, and then delve into the method described in this
   %paper, \textit{pyroprinting}, and how my work supports this newly developed
   %library-dependent MST technique.
   %
   %Among the various MST techniques described in Sargeant's review, the most
   %important and relevant technique to the work described in this paper are
...
\chapter{Related Work}\label{chap:related}
   ...
   %Incremental Clustering relation%
